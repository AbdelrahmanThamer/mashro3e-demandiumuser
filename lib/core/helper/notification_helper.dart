...
class NotificationHelper {
  ...
  static Future<void> initialize(FlutterLocalNotificationsPlugin flutterLocalNotificationsPlugin) async {
    ...
    FirebaseMessaging.onMessage.listen((RemoteMessage message) {
      printLog("onMessage: ${message.notification!.title}/${message.notification!.body}/${message.notification!.titleLocKey}");
      // NotificationHelper.showNotification(message, flutterLocalNotificationsPlugin, false);
      if(Get.find<AuthController>().isLoggedIn()) {
        Get.find<NotificationController>().getNotifications(1);
      }
      Get.dialog(PushNotificationDialog(title: message.notification!.title, body: "message.notification!.body", bookingID: message.data['booking_id'].toString()));
    });
    ...
    if(!GetPlatform.isIOS) {
      ...
      if(_image != null && _image.isNotEmpty) {
        try{
          ...
          await showBigTextNotification(_title!, '  ', _orderID!, fln);
        }
      }else {
  ...
  static Future<void> showTextNotification(String title, String body, String orderID, FlutterLocalNotificationsPlugin fln) async {
    const AndroidNotificationDetails androidPlatformChannelSpecifics = AndroidNotificationDetails(
      'ondemand', 'ondemand', playSound: true,
  ...
  static Future<void> showBigTextNotification(String title, String body, String orderID, FlutterLocalNotificationsPlugin fln) async {
    ...
    AndroidNotificationDetails androidPlatformChannelSpecifics = AndroidNotificationDetails(
      'ondemand', 'ondemand', importance: Importance.max,
