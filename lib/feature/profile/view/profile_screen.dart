...
class ProfileScreen extends GetView<UserController> {
  ...
  Widget build(BuildContext context) {
    ...
    final _profileCartModelList = [
      ...
      ProfileCardItemModel(
        'settings'.tr, Images.settings, RouteHelper.getSettingRoute(),
      ),
                      ...
                      return ProfileCardItem(
                        ...
                        onTap: () {
                          if(_profileCartModelList[index].routeName == 'sign_out'){
                            if(Get.find<AuthController>().isLoggedIn()) {
                              Get.dialog(ConfirmationDialog(icon: Images.logoutIcon, description: 'are_you_sure_to_logout'.tr, isLogOut: true, onYesPressed: ()async {
